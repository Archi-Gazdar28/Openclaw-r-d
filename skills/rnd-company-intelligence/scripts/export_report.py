#!/usr/bin/env python3
"""
export_report.py  —  OpenClaw R&D Intelligence Report Exporter  v3.2
Generates a self-contained HTML report with embedded charts optimized for precise PDF conversion.
...
section{{margin-bottom:2.8rem; clear: both;}}
...
<footer style="margin-top:3rem;padding-top:1rem;border-top:1px solid var(--rule);font-size:12px;color:var(--ink4);text-align:center;page-break-inside:avoid;">
...
# ── CLI ───────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="OpenClaw report exporter v3.2")
